...
function resolveGeminiModels(rawModel: string): string[] {
  const preferred = rawModel.trim();
  const uniq = new Set<string>();
  if (preferred) uniq.add(preferred);
  for (const model of GEMINI_MODEL_FALLBACKS) uniq.add(model);
  return [...uniq];
}
...
function isModelNotFoundError(err: any): boolean {
  ...
  const text = `${String(err?.message || '')}\n${String(err?.body || '')}`.toLowerCase();
  return /model|not found|not supported|generat(e|econtent)|listmodels/.test(text);
}
...
export const GeminiBot = (o: { apiKey: string; model?: string }): BotFunc =>
  async (ctx: BotCtx) => {
    try {
      ...
      const modelCandidates = resolveGeminiModels(model);
      const phase = ((ctx as any)?.phase || 'play') as 'bid' | 'double' | 'play';
      const attempts: PromptMode[] = ['normal', 'safe', 'minimal'];
      let lastErr: any;

      for (const modelName of modelCandidates) {
        const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(modelName)}:generateContent?key=${encodeURIComponent(o.apiKey)}`;
          ...
          try {
            ...
            if (phase === 'bid') {
              if (typeof p.bid === 'boolean') {
                return { phase: 'bid', bid: !!p.bid, reason: nonEmptyReason(p.reason, `Gemini/${modelName}`) };
              }
              if (p.move === 'pass') return { phase: 'bid', bid: false, reason: nonEmptyReason(p.reason, `Gemini/${modelName}`) };
              if (p.move === 'play') return { phase: 'bid', bid: true, reason: nonEmptyReason(p.reason, `Gemini/${modelName}`) };
              throw new Error('invalid bid response');
            }
            if (phase === 'double') {
              if (typeof p.double === 'boolean') {
                return { phase: 'double', double: !!p.double, reason: nonEmptyReason(p.reason, `Gemini/${modelName}`) };
              }
              if (typeof p.bid === 'boolean') {
                return { phase: 'double', double: !!p.bid, reason: nonEmptyReason(p.reason, `Gemini/${modelName}`) };
              }
              if (p.move === 'pass') return { phase: 'double', double: false, reason: nonEmptyReason(p.reason, `Gemini/${modelName}`) };
              if (p.move === 'play') return { phase: 'double', double: true, reason: nonEmptyReason(p.reason, `Gemini/${modelName}`) };
              throw new Error('invalid double response');
            }
            const mv = p.move === 'pass' ? 'pass' : 'play';
            const cards: string[] = Array.isArray(p.cards) ? p.cards : [];
            const reason = nonEmptyReason(p.reason, `Gemini/${modelName}`);
            return mv === 'pass'
